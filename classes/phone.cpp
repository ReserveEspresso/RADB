...
//    }
    ...
    while (outputLines.length()>0)
    {
        ...
        tmp.remove(QRegularExpression("\\s+$"));
        lineParts=tmp.split(QRegularExpression("\\s+"));
        if (lineParts.length()>8)
        {
            ...
            else if (name.contains("1;37"))//white
            {
                ...
                tmpFile.fileColor = QColor(Qt::black);
            name.remove(QRegularExpression("\\[\\d;\\d+m"));
...
FileList *Phone::getStaticFileList(QString path, QString sdk, bool hiddenFiles)
{
    ...
    while (outputLines.length()>0)
    {
        lineParts.clear();
        name.clear();
        lineParts=outputLines.first().split(QRegularExpression("\\s+"));
        outputLines.removeFirst();
        if (lineParts.length()>8)
        {
            if (lineParts[4].contains(","))
...
bool Phone::makeDir(QString newDir)
{
    ...
    if (outputLine.contains(QRegularExpression("can.+t create directory")))
        return false;
    return true;
}
...
//su -c 'busybox grep gmail-ls /data/system/sync/accounts.xml'
    ...
    {
        element.remove(QRegularExpression(".*account=\""));
        element.remove(QRegularExpression("\".*"));
...
{//? (192.168.2.1) at d8:5d:4c:de:85:94 [ether]  on tiwlan0
    ...
    proces->waitForFinished(-1);
    tmp = proces->readAll();
    tmp.remove(QRegularExpression(".*inet addr:"));
    tmp.remove(QRegularExpression("\\s.*"));
